...
type AppTasks = "beginLoadConference" | "beginLoadCurrentUser" | "loadingConference" | "loadingCurrentUser";
...
export default function App() {
                        ...
                        if (appState.conferenceId) {
                            if (user && user.id) {
                                ...
                                if (!(await cache.Ready)) {
                                    ...
                                    addError(
                                        "Conference not available. Please go back to the home page and select a conference."
                                    );
                                    dispatchAppUpdate([
                                        {
                                    ...
                                    const { admin: _isAdmin, manager: _isManager } = await _Role.isUserInRoles(
                                        user.id,
                                        appState.conferenceId,
                                        ["admin", "manager"]
                                    );
                                    isAdmin = _isAdmin;
                                    ...
                                    if (profile) {
                                        ...
                                        if (profile.isBanned) {
                                            ...
                                            LocalStorage_Conference.wasBannedFromName =
                                                appState.conference?.name ?? "the selected conference";
                                            addError(
                                                "Conference not available. Please go back to the home page and select a conference."
                                            );
                                            dispatchAppUpdate([
                        ...
                        dispatchAppUpdate([
                            {
                                ...
                                data: profile && sessionToken ? { profile, sessionToken } : null,
                            },
                            {
                                action: "setUserRoles",
    ...
    const onConferenceUpdated = useCallback(
        function _onConferenceUpdated(value: DataUpdatedEventDetails<"Conference">) {
            for (const object of value.objects) {
                if (appState.conference && object.id === appState.conference.id) {
                    const newConf = object as Conference;
                    if (
                        handleParseFileURLWeirdness(appState.conference.headerImage) !==
                            handleParseFileURLWeirdness(newConf.headerImage) ||
    ...
    const onUserProfileUpdated = useCallback(
        async function _onUserProfileUpdated(value: DataUpdatedEventDetails<"UserProfile">) {
            for (const object of value.objects) {
    ...
    const doLogin = useCallback(
        async function _doLogin(email: string, password: string): Promise<boolean> {
            try {
                ...
                const { admin: isAdmin, manager: _isManager } = await _Role.isUserInRoles(
                    parseUser.user.id,
                    appState.conference.id,
                    ["admin", "manager"]
                );
                ...
                dispatchAppUpdate([
                    {
                        action: "setUserProfile",
                        data: profile ? { profile, sessionToken: parseUser.sessionToken } : null,
                    },
                    {
                        action: "setUserRoles",
                        roles: { isAdmin, isManager },
    ...
    const doLogout = useCallback(
        ...
        [appState.conference, history]
    );

    const failedToLoadConferences = useCallback(
        async function _failedToLoadConferences(reason: any): Promise<void> {
            ...
            if (typeof reason === "string") {
                if (reason.match(/ParseError: 209 Invalid session token/gi)?.length) {
                    handled = true;
    ...
    const selectConference = useCallback(
        async function _selectConference(id: string | null): Promise<boolean> {
    ...
    const toggleSidebar = useCallback(
        async function _toggleSidebar(): Promise<void> {
