...
export default function AdminRegistration() {
    ...
    useSafeAsync(
        async () => Registration.getAll(conference.id),
        setExistingRegs,
        [conference.id],
    ...
    function sendRegistrationEmails(sendOnlyUnsent: boolean): void {
        doSendRegistrationEmails({ sendOnlyUnsent, conference: conference.id })
            .then(response => {
                if (response.success) {
                    addNotification(
                        `Sent ${response.results.length} registration ${
                            response.results.length === 1 ? "email" : "emails"
                        }.`
                    );
                } else {
                    addError(
                        `Failed to send ${response.results.filter(result => !result.success).length} of ${
    ...
    const adminPanel = (
        <>
            ...
            <p>
                ...
                not be created for existing email addresses.
            </p>
            <div>
                {isUploading ? (
                    <LoadingSpinner message="Uploading registrations" />
                ) : (
                    <>
                        <div>
                            <label htmlFor="newRegistrationsFile">Select JSON file</label>
                            <br />
                            <input
                                id="newRegistrationsFile"
                                type="file"
            ...
            <p>
                <pre>
                    {`type registrationsFile = Array<RegistrationSpec>;
...
type RegistrationSpec = {
    ...
    newRole: "attendee" | "manager" | "admin";
            ...
            <p>
                Send a repeat registration email to all users that have not yet registered. Includes users that have not
                yet received any registration email.
            </p>
            <p>
                {sentRepeats ? (
                    <>
                        You have already sent repeat emails. Pressing this button again will send the same people
                        another copy of their registration email. If this is what you would like to do, please refresh
                        the page.
                    </>
                ) : isSending ? (
                    <LoadingSpinner message="Sending emails" />
