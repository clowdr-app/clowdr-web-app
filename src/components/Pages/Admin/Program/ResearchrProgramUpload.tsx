            ...
            if (!skipSession) {
                ...
                for (const dataTimeslot of dataTimeslots) {
                    if (dataTimeslot.getElementsByTagName("event_id").length > 0) {
                        const dataTimeslotTrackEls = dataTimeslot
                            .getElementsByTagName("tracks")[0]
                            .getElementsByTagName("track");
                                ...
                                for (const personEl of dataPersonEls) {
                                    ...
                                    if (role === "Session Chair") {
                                        ...
                                        outputPersons[concatName] = {
                                            name: concatName,
                                            affiliation:
                                                personEl.getElementsByTagName("affiliation")[0].textContent ??
                                                undefined,
                                        };
...
export default function AdminResearchrProgramUpload() {
    ...
    useSafeAsync(
        ...
        setUploadProgress,
        [conference.id, lastUpdateTime],
        "ResearchrProgramUpload:setUploadProgress"
    );
    ...
    useEffect(() => {
        try {
            ...
            if (inputProgramDatas) {
                _trackNames = processResearchrXMLsForDefaults(
                    inputProgramDatas,
                    roomNamesToInclude
                ).trackNames.sort((x, y) => x.localeCompare(y));
            }
            ...
            if (addedTrackNames.length > 0) {
                setTrackVideoRoomDefaults(oldDefaults =>
                    oldDefaults.filter(x => _trackNames.includes(x)).concat(addedTrackNames)
                );
                setTrackTextChatDefaults(oldDefaults =>
                    oldDefaults.filter(x => _trackNames.includes(x)).concat(addedTrackNames)
                );
                setTrackExhibitDefaults(oldDefaults => oldDefaults.filter(x => _trackNames.includes(x)));
            }
        } catch (e) {
    ...
    useEffect(() => {
        setTrackTextChatDefaults(oldDefaults =>
            Array.from(new Set(oldDefaults.concat(trackVideomRoomDefaults)).values())
        );
    ...
    const selectedEvent =
        selectedEventId && selectedEventId !== "none" ? programData?.events[selectedEventId] : undefined;
    ...
    return uploadProgress === null ? (
        ...
        <div className="admin-program-upload">
            {uploadProgress === false ? (
                <>
                    <div>
                        <label htmlFor="newUploadDataFile">Select Researchr Program XML file</label>
                        <br />
                        <input
                            id="newUploadDataFile"
                            type="file"
                    ...
                    <br />
                    <div>
                        <label htmlFor="newUploadFeedsFile">Select feeds JSON file</label>
                        <br />
                        <input
                            id="newUploadFeedsFile"
                            type="file"
                    ...
                    <br />
                    <label id="rooms_to_include_label">
                        Researchr rooms to include: Select "rooms" whose events should be included in the upload:
                    </label>
                    <MultiSelect
                        ...
                        onChange={(vals: { label: string; value: string }[]) => {
                            setRoomNamesToInclude(new Set(vals.map(x => x.value)));
                        }}
                    />
                    <br />
                    <label id="track_video_defaults_label">
                        Track defaults: Select tracks whose events should have video rooms by default:
                    </label>
                    <MultiSelect
                    ...
                    />
                    <br />
                    <label id="track_text_defaults_label">
                        Track defaults: Select tracks whose events should have text chats by default:
                    </label>
                    <MultiSelect
                    ...
                    />
                    <br />
                    <label id="track_exhibit_defaults_label">
                        Track defaults: Select tracks whose events should be exhibited by default:
                    </label>
                    <MultiSelect
                        ...
                        value={trackExhibitDefaults
                            ...
                            .map(x => ({ value: x, label: x }))}
                        onChange={(vals: { label: string; value: string }[]) => {
                            setTrackExhibitDefaults(Array.from(new Set(vals.map(x => x.value))));
                        }}
                    />
                    {!!programData ? (
                        <>
                            ...
                            <div>
                                <label id={`tracks-select`}>Select a track:</label>
                                <br />
                                <select
                                ...
                                >
                                    <option disabled value="none" key="delete">
                                        {" "}
                                        -- select an option --{" "}
                                    </option>
                            ...
                            <hr />
                            {selectedTrackName &&
                            Object.values(programData.sessions).filter(x => x?.track === selectedTrackName).length >
                                0 ? (
                                <>
                                    <div>
                                        <label id={`sessions-select`}>Select a session from the selected track:</label>
                                        <br />
                                        <select
                                        ...
                                        >
                                            ...
                                            {Object.values(programData.sessions)
                                                ...
                                                .map(session => {
                                                    if (session) {
                                                        const events = removeUndefined(
                                                            Object.values(programData.events).filter(
                                                                x => x?.session === session.id
                                                            )
                                                        );

                                                        const earliestStart = events?.reduce(
                                                            (r, e) =>
                                                                r.getTime() < e.startTime.getTime() ? r : e.startTime,
                                                            new Date(32503680000000)
                                                        );
                                                        const latestEnd = events?.reduce(
                                                            (r, e) =>
                                                                r.getTime() > e.endTime.getTime() ? r : e.endTime,
                                                            new Date(0)
                                                        );

                                                        return (
                                                            <option key={session.id} value={session.id}>
                                                                {session.title} ({fmtDate(earliestStart)}{" "}
                                                                {fmtTime(earliestStart)} - {fmtDate(latestEnd)}{" "}
                                                                {fmtTime(latestEnd)})
                                                            </option>
                                                        );
                                                    }
                                    ...
                                    <br />
                                    <div>
                                        <label id={`events-select`}>Select an event from the selected session:</label>
                                        <br />
                                        <select
                                                    ...
                                                    if (event) {
                                                        ...
                                                        return (
                                                            <option key={event.id} value={event.id}>
                                                                {item.title} ({fmtDate(event.startTime)}{" "}
                                                                {fmtTime(event.startTime)} - {fmtDate(event.endTime)}{" "}
                            ...
                            Object.values(programData.sessions).filter(x => x?.track === selectedTrackName).length ===
                                ...
                                <>
                                    <div>
                                        <label id={`items-select`}>Select an item from the selected track:</label>
                                        <br />
                                        <select
                    ...
                    <AsyncButton
                        ...
                        action={async () => {
                            ...
                            try {
                                assert(
                                    await Parse.Cloud.run("import-program", {
                                        conference: conference.id,
                                        data: programData,
                                    })
                                );
