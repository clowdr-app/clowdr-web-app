...
type ProgramGroupTasks = "loadingSessionsAndEvents";
...
async function filterSessionsAndEvents(
    ...
    _search: string | null,
    minSearchLength: number
): Promise<[Array<ProgramSessionWithStartEnd>, Array<ProgramSessionEvent>]> {
    if (_search && _search.length >= minSearchLength) {
        const search = _search.toLowerCase();

        const sessions: Array<ProgramSessionWithStartEnd> = removeNull(
            await Promise.all(
                allSessions.map(async x => {
        ...
        const events: Array<ProgramSessionEvent> = removeNull(
            await Promise.all(
                allEvents.map(async x => {
                    ...
                    if (
                        !!itemTitle.toLowerCase().match(search)?.length
                        // || !!trackName.toLowerCase().match(search)?.length
                        // || !!sessionTitle.toLowerCase().match(search)?.length
...
function nextSidebarState(
    currentState: ProgramGroupState,
    updates: ProgramGroupUpdate | Array<ProgramGroupUpdate>
): ProgramGroupState {
...
export default function ProgramGroup(props: Props) {
    ...
    const [state, dispatchUpdate] = useReducer(nextSidebarState, {
        tasks: new Set(["loadingSessionsAndEvents"] as ProgramGroupTasks[]),
        ...
        async function updateSessionsAndEvents() {
            try {
                const promises: [Promise<Array<ProgramSession>>, Promise<Array<ProgramSessionEvent>>] = [
                    ProgramSession.getAll(conf.id),
                    ProgramSessionEvent.getAll(conf.id),
                ];
    ...
    const notifyWatchedItems = useCallback(
        async function _notifyWatchedItems() {
            const now = Date.now();
                    ...
                    if (!state.notifiedSessionIds.includes(session.id)) {
                        ...
                        if (dist < limit) {
                            ...
                            if (!location.pathname.includes(path)) {
                                if (
                                    state.watchedSessionIds?.includes(session.id) ||
                    ...
                    if (!state.notifiedEventIds.includes(event.id)) {
                        ...
                        if (dist < 1000 * 60 * 1.5) {
                            ...
                            if (!location.pathname.includes(path)) {
                                ...
                                if (
                                    state.watchedEventIds?.includes(event.id) ||
        ...
        [
            ...
            state.notifiedSessionIds,
            state.sessions,
            state.watchedEventIds,
            state.watchedSessionIds,
            state.watchedTrackIds,
        ]
    );
    ...
    const sessionsWSE: ProgramSessionWithStartEnd[] | null = useMemo(() => {
        if (state.sessions && state.events) {
            ...
            return state.sessions.map(session => {
                ...
                return {
                    session,
                    earliestStart: eventsOfSession.reduce(
                        (r, e) => (r.getTime() < e.startTime.getTime() ? r : e.startTime),
                        new Date(32503680000000)
                    ),
                    latestEnd: eventsOfSession.reduce(
    ...
    useEffect(() => {
        ...
        async function updateFiltered() {
            try {
                ...
                const [filteredSessions, filteredEvents] = await promise.promise;
                dispatchUpdate([
    ...
    if (mUser) {
        ...
        if (state.sessions && state.events && (state.filteredSessions.length > 0 || state.filteredEvents.length > 0)) {
            const programTimeBoundaries: Array<number> = [0.1, 0.5, 3, 15, 30, 60, 120];
            ...
            const sessions =
                state.programSearch && state.programSearch.length >= props.minSearchLength
                    ...
                    : state.filteredSessions.filter(
                          x => x.latestEnd.getTime() >= endLimit && x.earliestStart.getTime() <= startLimit
                      );
            ...
            const events =
                state.programSearch && state.programSearch.length >= props.minSearchLength
                    ...
                    : state.filteredEvents.filter(
                          x => x.endTime.getTime() >= endLimit && x.startTime.getTime() <= startLimit
                      );
        ...
        if (!_program) {
            _program = (
                <>
                    {state.sessions && state.events ? (
                        <span className="menu-group">No upcoming events in the next 2 hours.</span>
                    ) : (
                        <LoadingSpinner />
                    )}
                    <MenuGroup
                        items={[
                            {
                                ...
                                element: (
                                    <MenuItem
                                        title="View whole program"
                                        label="View whole program"
